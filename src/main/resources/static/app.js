...
async function main() {
    ...
    if (currentUser) {
        // Attach event listeners for forms only if user is logged in
...
function renderUI() {
    ...
    document.querySelectorAll('main section').forEach(s => s.classList.add('hidden'));
    publicBookViewSection.classList.remove('hidden'); // Public book list is always visible
    ...
    if (currentUser) {
        ...
        switch (currentUser.role) {
            ...
            case 'PERSONNEL':
                ...
                break;
            case 'ADMIN':
                personnelDashboardSection.classList.remove('hidden'); // Admin can do everything a personnel can
    ...
    try {
        ...
        if (requests.length === 0) {
            ...
            requests.forEach(req => {
                tableHTML += `<tr><td>${req.book.title}</td><td>${req.requestDate}</td><td class="status-${req.status.toLowerCase()}">${req.status}</td></tr>`;
            });
        }
        tableHTML += '</tbody></table>';
...
// --- PERSONNEL/ADMIN-SPECIFIC FUNCTIONS ---
...
async function handleProfilePictureUpload(event) {
    ...
    try {
        ...
        if (response.ok) {
            alert('Profile picture uploaded successfully! The page will now reload.');
...
async function handleCreateBook(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            addBookForm.reset();
            loadAdminBooks();
            loadPublicBooks(); // Refresh public list too
        } else {
